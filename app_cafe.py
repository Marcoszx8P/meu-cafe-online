...
import requests
from bs4 import BeautifulSoup # Necessário para ler o Investing.com
import base64
import os
...
def buscar_londres_investing():
    ...
    try:
        ...
        # Captura o preço e a variação (os IDs/Classes podem mudar, mas estes são os atuais)
        ...
        # Limpa o texto da variação (ex: "-0,50%") para float (ex: -0.005)
...
def buscar_mercado():
    try:
        # Nova York e Dólar continuam via Yahoo (mais estável)
...
# --- 3. FUNÇÃO DE ESTILO E FUNDO --- (Mantida igual ao seu original)
...
with exp_col1:
    ...
    st.write("Cotações oficiais de Bebida Dura, Rio e Conilon do CCCV (Vitória/ES).")
with exp_col2:
    st.markdown("**2. Variação Combinada**")
    st.write("Monitoramento de NY (Arábica), **Londres (Conilon vía Investing.com)** e Dólar.")
with exp_col3:
    st.markdown("**3. Alvo Estimado**")
    st.write("Cálculo da tendência do mercado físico baseado na oscilação internacional.")
...
if ny_p == 0:
    ...
    c2.metric("Londres (Investing)", f"US$ {ld_p:.0f}/t", f"{ld_v:.2%}") # DADOS DO INVESTING
    ...
    # CONILON (Calculado com dados do Investing)
